...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-slate-900 text-slate-100 font-sans overflow-hidden">
      ...
      {/* --- SIDEBAR IZQUIERDO: AHORA CON ANCHO FIJO (w-80) --- */}
      <aside className="w-80 flex-shrink-0 bg-slate-800/40 backdrop-blur-2xl border-r border-slate-700/50 flex flex-col z-20 shadow-2xl">
        ...
        {/* Tabs Principales */}
        <div className="flex p-2 bg-slate-900/40 m-4 rounded-xl gap-1 border border-slate-700/30">
          {[
                ...
                {blocks.map((block, idx) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={(e) => { e.stopPropagation(); moveBlock(idx, 'up'); }} className="p-1.5 hover:bg-slate-600 rounded"><MoveUp size={14}/></button>
                      <button onClick={(e) => { e.stopPropagation(); deleteBlock(block.id); }} className="p-1.5 hover:bg-red-500/20 text-red-400 rounded"><Trash2 size={14}/></button>
                    </div>
                  </div>
                ))}
              </div>
                      ...
                      {key === 'description' || key === 'text' ? (
                        ...
                        <div className="space-y-3">
                          <div className="h-32 bg-slate-900 rounded-xl overflow-hidden border border-dashed border-slate-700 flex items-center justify-center group relative">
                            <img src={selectedBlock.content[key]} className="absolute inset-0 w-full h-full object-cover opacity-50" />
      ...
      <main className="flex-1 relative flex flex-col items-center bg-slate-950 overflow-hidden">
        
        {/* Pattern de fondo profesional */}
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>

        {/* Header de Preview */}
        <header className="w-full h-16 border-b border-slate-800/50 flex items-center justify-between px-8 bg-slate-900/60 backdrop-blur-xl z-10">
          <div className="flex items-center gap-6">
            ...
            <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Vista previa en tiempo real</p>
          </div>

          <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-slate-400 hover:text-white transition-colors">
              <Eye size={16} /> Vista Invitado
            </button>
            <button className="flex items-center gap-2 px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-black rounded-xl shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
        ...
        {/* El Canvas / Simulador */}
        <div className="flex-1 w-full overflow-y-auto p-12 flex justify-center custom-scrollbar scroll-smooth">
          <div 
            className={`transition-all duration-700 ease-[cubic-bezier(0.23, 1, 0.32, 1)] relative ${
              isMobilePreview 
                ? 'w-[375px] h-fit min-h-[812px] rounded-[3.5rem] border-[12px] border-slate-800 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] ring-1 ring-slate-700 ring-offset-4 ring-offset-slate-900' 
                : 'w-full max-w-5xl h-fit min-h-screen bg-white rounded-2xl shadow-2xl'
            } bg-white overflow-hidden`}
          >
            {/* Notch de iPhone */}
            {isMobilePreview && (
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-36 h-7 bg-slate-800 rounded-b-3xl z-50 flex items-center justify-center">
            ...
            {isMobilePreview && musicConfig.enabled && (
              <div className="absolute bottom-8 right-6 z-40 animate-bounce">
                <button 
            ...
            <div className="flex flex-col">
              {blocks.map((block) => (
                <div 
                  ...
                  className={`relative group cursor-pointer transition-all duration-300 ${
                    selectedBlockId === block.id 
                      ? 'ring-4 ring-pink-500/30 z-10' 
                      : 'hover:bg-slate-50'
                  }`}
                >
                  {/* Controles Flotantes Rápidos (Solo aparecen al seleccionar o hover) */}
                  <div className={`absolute top-4 right-4 flex gap-1 z-20 transition-all duration-300 ${selectedBlockId === block.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 group-hover:opacity-100'}`}>
                    <div className="flex bg-white/90 backdrop-blur-md shadow-xl border border-slate-200 rounded-lg p-1">
                      <button className="p-1.5 hover:bg-slate-100 rounded text-slate-600"><MoveUp size={14}/></button>
                      <button className="p-1.5 hover:bg-slate-100 rounded text-slate-600"><MoveDown size={14}/></button>
                  ...
                  <div className={`absolute -top-3 left-8 px-3 py-1 bg-pink-500 text-white text-[9px] font-black rounded-full shadow-lg transition-all z-20 uppercase tracking-widest ${selectedBlockId === block.id ? 'opacity-100' : 'opacity-0'}`}>
                    Editando: {block.type}
                  </div>
              ...
              <div 
                onClick={() => setIsAddingBlock(true)}
                className="py-16 border-2 border-dashed border-slate-200 m-6 rounded-3xl flex flex-col items-center justify-center text-slate-300 hover:text-pink-500 hover:border-pink-300 hover:bg-pink-50 transition-all cursor-pointer group"
              >
                <div className="w-14 h-14 bg-slate-50 group-hover:bg-pink-100 rounded-full flex items-center justify-center transition-all mb-4">
                  <Plus size={32} />
                </div>
                <span className="font-black text-xs uppercase tracking-widest">Pulsa para añadir más contenido</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {isAddingBlock && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
            ...
            <div className="relative bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] p-10 max-h-[85vh] overflow-hidden flex flex-col scale-in-center">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-3xl font-black tracking-tight">Agregar Bloque</h2>
                  <p className="text-slate-500 text-sm mt-1">Personaliza tu invitación con módulos interactivos.</p>
                </div>
                <button onClick={() => setIsAddingBlock(false)} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-2xl transition-all">
                  <X size={20} />
                </button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 overflow-y-auto pr-2 custom-scrollbar pb-4">
                {BLOCK_TYPES.map(item => (
                  <button
                    key={item.type}
                    onClick={() => addBlock(item.type)}
                    className="flex flex-col items-center gap-4 p-6 bg-slate-800/40 border border-slate-700/50 rounded-3xl hover:border-pink-500 hover:bg-slate-800 transition-all text-center group active:scale-95"
                  >
                    <div className="w-14 h-14 bg-slate-800 group-hover:bg-pink-500 text-slate-400 group-hover:text-white rounded-2xl flex items-center justify-center transition-all shadow-lg">
                      {item.icon}
                    </div>
                    <div>
                      <p className="font-bold text-sm tracking-tight">{item.label}</p>
                      <p className="text-[10px] text-slate-500 mt-2 leading-tight">{item.description}</p>
                    </div>
                  </button>
                ))}
              </div>
        ...
        {showTutorial && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-sm">
            <div className="bg-white text-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl p-8 animate-in zoom-in duration-300 relative overflow-hidden">
              ...
              <div className="mt-4 text-center">
                <div className="w-16 h-16 bg-pink-50 text-pink-500 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  {tutorialStep === 1 && <Smartphone size={32} />}
                  {tutorialStep === 2 && <Layers size={32} />}
                  {tutorialStep === 3 && <Plus size={32} />}
                  {tutorialStep === 4 && <Share2 size={32} />}
                </div>

                <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mb-2">Paso {tutorialStep} de 4</p>
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    ...
                    className="flex-[2] py-4 bg-pink-500 hover:bg-pink-600 text-white font-black rounded-2xl shadow-xl shadow-pink-500/20 transition-all flex items-center justify-center gap-2"
                  >
      ...
      <style>{`
        ...
        .scale-in-center { animation: scale-in-center 0.4s cubic-bezier(0.23, 1.0, 0.32, 1.0) both; }
      `}</style>
    </div>
  );
}
...
function BlockRenderer({ type, content }) {
  switch(type) {
    case 'hero':
      return (
        <div className="relative h-[600px] w-full overflow-hidden flex items-end p-10 bg-slate-100">
          ...
          <div className="relative text-white z-10 w-full text-center">
            <p className="text-[10px] tracking-[0.4em] uppercase font-bold mb-4 animate-pulse">01 . 07 . 2026</p>
            <h2 className="text-5xl font-serif mb-6 leading-tight drop-shadow-2xl">{content.names}</h2>
            <div className="h-px w-24 bg-white/40 mx-auto mb-6"></div>
            <p className="text-xs tracking-[0.2em] uppercase font-light opacity-80 italic">Te invitamos a celebrar con nosotros</p>
          </div>
        </div>
      );
    ...
    case 'countdown':
      return (
        <div className="py-16 px-6 bg-[#fdfaf5] text-center border-y border-amber-100/50">
          <p className="text-[10px] uppercase tracking-[0.3em] text-amber-800/60 font-black mb-8">{content.title}</p>
          <div className="flex justify-center gap-3">
            {[
              {v: '91', l: 'Días'}, {v: '06', l: 'Hrs'}, {v: '14', l: 'Min'}, {v: '11', l: 'Seg'}
            ].map(t => (
              <div key={t.l} className="flex flex-col items-center">
                <div className="w-16 h-16 bg-white shadow-xl shadow-amber-900/5 border border-amber-100 rounded-2xl flex items-center justify-center text-2xl font-black text-amber-900 mb-2">
                  {t.v}
                </div>
                <span className="text-[9px] uppercase text-amber-800/40 font-black tracking-widest">{t.l}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    case 'details':
      return (
        <div className="py-24 px-12 text-center bg-white relative overflow-hidden">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 bg-pink-50 rounded-full blur-[100px] opacity-30"></div>
          <h3 className="text-3xl font-serif text-slate-800 mb-8">{content.title}</h3>
          <p className="text-slate-500 text-sm leading-relaxed italic mb-10 max-w-xs mx-auto">{content.description}</p>
          <div className="inline-flex items-center gap-3 py-4 px-8 bg-slate-50 rounded-2xl text-slate-700 text-xs font-bold border border-slate-100 shadow-sm">
    ...
    case 'map':
      return (
        <div className="p-6 bg-slate-50">
          <div className="bg-slate-200 h-64 rounded-[2rem] flex items-center justify-center text-slate-400 overflow-hidden relative shadow-inner border border-slate-200">
            ...
            <div className="relative bg-white/90 backdrop-blur-md p-5 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex items-center gap-4 border border-white">
              <div className="w-12 h-12 bg-pink-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-pink-500/20">
                <MapPin size={24} />
              </div>
              <div className="text-left">
                <p className="text-xs font-black text-slate-800 uppercase tracking-tight">{content.title}</p>
                <p className="text-[10px] text-slate-500 mt-0.5">{content.address}</p>
              </div>
    ...
    case 'rsvp':
      return (
        <div className="p-10 bg-slate-900 text-white text-center relative">
          ...
          <h4 className="font-bold text-xl mb-6 relative z-10 tracking-tight">{content.title}</h4>
          <div className="space-y-4 relative z-10 max-w-xs mx-auto">
            <input placeholder="Nombre completo" className="w-full p-4 bg-white/10 border border-white/20 rounded-2xl text-sm outline-none focus:border-pink-500 transition-all placeholder:text-white/30" />
            <div className="grid grid-cols-2 gap-3">
              <button className="py-4 bg-pink-500 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-lg shadow-pink-500/20">Sí, iré</button>
              <button className="py-4 bg-white/10 text-white/60 font-black rounded-2xl text-[10px] uppercase tracking-widest border border-white/10">No podré</button>
            </div>
          </div>
        </div>
      );
    ...
    default:
      return <div className="p-12 text-center text-slate-400 font-bold bg-slate-50 uppercase tracking-widest text-[10px]">Bloque: {type}</div>;
  }
}